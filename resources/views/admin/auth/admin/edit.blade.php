...
@section('breadcrumb')
    ...
    <li><a href="{{ route('admin.create') }}">ویرایش</a></li>
@endsection
...
<div class="col-12">
    <div class="portlet box shadow">
        ...
        <div class="portlet-body">
            <form role="form" method="POST" action="{{ route('admin.update', [$admin]) }}">
                @csrf
                @method('PUT')
                ...
                <div class="form-body">
                    <div class="form-group">
                        ...
                        <div class="input-group @error('username') has-error @enderror">
                            ...
                            <input type="text" class="form-control" placeholder="نام کاربری" value="{{ old('username', $admin->username) }}" name="username">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group @error('phone') has-error @enderror">
                            ...
                            <input type="text" class="form-control" placeholder="شماره تماس" value="{{ old('phone', $admin->phone) }}" name="phone">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group @error('status') has-error @enderror">
                            ...
                            <select name="status" class="form-select">
                                <option value="0" {{ old('status', $admin->status) == '0' ? 'selected' : '' }}>غیر فعال</option>
                                <option value="1" {{ old('status', $admin->status) == '1' ? 'selected' : '' }} selected>فعال</option>
                            </select>
                        ...
                        <div class="input-group @error('roles') has-error @enderror">
                            ...
                            <select name="roles[]" id="roles" class="form-control roles" multiple="multiple">
                                @foreach ($roles as $key => $role)
                                    <option {{ in_array($role->name, $admin->roles->pluck('name')->toArray()) ? 'selected' : '' }} value="{{ $role->name }}">{{ $role->name }}</option>
                                @endforeach
                            </select>
